...
// Helper: Check if client is running on GitHub Pages or HTTPS static host without live HTTPS API
function isStaticDemoMode() {
  if (typeof window !== 'undefined') {
    const isGithub = window.location.hostname.includes('github.io');
    const isHttpsLocalHost = window.location.protocol === 'https:' && API_BASE_URL.startsWith('http:');
    return isGithub || isHttpsLocalHost;
  }
  return false;
}
...
api.interceptors.request.use(
  (config) => {
    ...
    // On GitHub Pages or Mixed Content HTTPS -> Bypass remote HTTP call and resolve static demo data
    if (isStaticDemoMode()) {
      config.adapter = async (cfg) => {
        ...
        if (url.includes('/auth/register')) {
          ...
          return {
            data: { success: true, token: 'demo-jwt-token-2026-new', user: newUser },
            status: 200,
            statusText: 'OK',
            headers: {},
            config: cfg
        ...
        if (url.includes('/auth/me')) {
          ...
          return { data: { success: true, user: savedUser }, status: 200, statusText: 'OK', headers: {}, config: cfg };
        }
        ...
        if (url.includes('/drivers')) {
          const tenantDrivers = mockDrivers.filter(d => d.organization_id === orgId);
          return { data: { success: true, count: tenantDrivers.length, drivers: tenantDrivers, driver: tenantDrivers[0] }, status: 200, statusText: 'OK', headers: {}, config: cfg };
        }

        // 6. VEHICLES
        if (url.includes('/vehicles')) {
          const tenantVehicles = mockVehicles.filter(v => v.organization_id === orgId);
          return { data: { success: true, count: tenantVehicles.length, vehicles: tenantVehicles, vehicle: tenantVehicles[0] }, status: 200, statusText: 'OK', headers: {}, config: cfg };
        }

        // 7. TRIPS
        if (url.includes('/trips')) {
          const tenantTrips = mockTrips.filter(t => t.organization_id === orgId);
          return { data: { success: true, count: tenantTrips.length, trips: tenantTrips, trip: tenantTrips[0] }, status: 200, statusText: 'OK', headers: {}, config: cfg };
        }
        ...
        if (url.includes('/tracking')) {
          ...
          return { data: { success: true, vehicles: tenantVehicles, activeTrips: tenantTrips, geofences: tenantGeofences, recentEvents: tenantEvents }, status: 200, statusText: 'OK', headers: {}, config: cfg };
        }
        ...
        if (url.includes('/geofences')) {
          const tenantGeofences = mockGeofences.filter(g => g.organization_id === orgId);
          const tenantEvents = mockGeofenceEvents.filter(e => e.organization_id === orgId);
          return { data: { success: true, count: tenantGeofences.length, geofences: tenantGeofences, events: tenantEvents }, status: 200, statusText: 'OK', headers: {}, config: cfg };
        }
        ...
        if (url.includes('/incidents')) {
          ...
          return { data: { success: true, incidents: tenantIncidents, inspections: tenantInspections }, status: 200, statusText: 'OK', headers: {}, config: cfg };
        }

        return { data: { success: true, message: 'Operation completed in Demo Mode.' }, status: 200, statusText: 'OK', headers: {}, config: cfg };
      };
    }
