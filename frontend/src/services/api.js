...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL !== undefined ? process.env.NEXT_PUBLIC_API_URL : 'http://localhost:5000/api';
...
// Helper: Check if client is running in static client demo mode (GitHub Pages or empty API URL)
function isStaticDemoMode() {
  if (typeof window !== 'undefined') {
    const isGithub = window.location.hostname.includes('github.io');
    const isEmptyApi = !API_BASE_URL || API_BASE_URL === '' || API_BASE_URL === '/api';
    return isGithub || isEmptyApi;
  }
  return false;
}
...
// Request Interceptor: Attach JWT Bearer Token & Intercept GitHub Pages Static Requests
api.interceptors.request.use(
  (config) => {
    ...
    // On GitHub Pages or empty API -> Resolve static demo response in-memory (No CORS network requests sent)
    if (isStaticDemoMode()) {
      config.adapter = async (cfg) => {
        ...
        if (url.includes('/auth/login')) {
          let reqData = {};
          try { reqData = typeof cfg.data === 'string' ? JSON.parse(cfg.data) : (cfg.data || {}); } catch (e) {}
          ...
          return {
            data: {
              success: true,
              message: 'Logged in via Live GitHub Pages Demo Mode.',
        ...
        if (url.includes('/auth/register')) {
          ...
          return { data: { success: true, token: 'demo-jwt-token-2026-new', user: newUser }, status: 200, statusText: 'OK', headers: { 'Access-Control-Allow-Origin': '*' }, config: cfg };
        }
        ...
        if (url.includes('/auth/me')) {
          ...
          return { data: { success: true, user: savedUser }, status: 200, statusText: 'OK', headers: { 'Access-Control-Allow-Origin': '*' }, config: cfg };
        }
        ...
        if (url.includes('/drivers')) {
          const tenantDrivers = mockDrivers.filter(d => d.organization_id === orgId);
          return { data: { success: true, count: tenantDrivers.length, drivers: tenantDrivers, driver: tenantDrivers[0] }, status: 200, statusText: 'OK', headers: { 'Access-Control-Allow-Origin': '*' }, config: cfg };
        }

        // 6. VEHICLES
        if (url.includes('/vehicles')) {
          const tenantVehicles = mockVehicles.filter(v => v.organization_id === orgId);
          return { data: { success: true, count: tenantVehicles.length, vehicles: tenantVehicles, vehicle: tenantVehicles[0] }, status: 200, statusText: 'OK', headers: { 'Access-Control-Allow-Origin': '*' }, config: cfg };
        }

        // 7. TRIPS
        if (url.includes('/trips')) {
          const tenantTrips = mockTrips.filter(t => t.organization_id === orgId);
          return { data: { success: true, count: tenantTrips.length, trips: tenantTrips, trip: tenantTrips[0] }, status: 200, statusText: 'OK', headers: { 'Access-Control-Allow-Origin': '*' }, config: cfg };
        }
        ...
        if (url.includes('/tracking')) {
          ...
          return { data: { success: true, vehicles: tenantVehicles, activeTrips: tenantTrips, geofences: tenantGeofences, recentEvents: tenantEvents }, status: 200, statusText: 'OK', headers: { 'Access-Control-Allow-Origin': '*' }, config: cfg };
        }
        ...
        if (url.includes('/geofences')) {
          const tenantGeofences = mockGeofences.filter(g => g.organization_id === orgId);
          const tenantEvents = mockGeofenceEvents.filter(e => e.organization_id === orgId);
          return { data: { success: true, count: tenantGeofences.length, geofences: tenantGeofences, events: tenantEvents }, status: 200, statusText: 'OK', headers: { 'Access-Control-Allow-Origin': '*' }, config: cfg };
        }
        ...
        if (url.includes('/incidents')) {
          ...
          return { data: { success: true, incidents: tenantIncidents, inspections: tenantInspections }, status: 200, statusText: 'OK', headers: { 'Access-Control-Allow-Origin': '*' }, config: cfg };
        }

        return { data: { success: true, message: 'Operation completed in Demo Mode.' }, status: 200, statusText: 'OK', headers: { 'Access-Control-Allow-Origin': '*' }, config: cfg };
      };
    }
